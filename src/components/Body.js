  ...
  return resList.length===0?<Shimmer/>:(
    <div className='m-4'>
        ...
        <button className="bg-teal-500 mx-4 text-white rounded-lg px-4 py-2 cursor-pointer "onClick={()=>handleSearch()}>Search</button>
         ...
         }className="bg-blue-500 text-white rounded-lg px-4 py-2 cursor-pointer ">Filter</button>
        </div>
        <div className='flex flex-wrap'>
            {filterItems.map((resCard)=>(<Link key={resCard.info.id}  to={"/restaurant/"+resCard.info.id}><RestaurantCard resData={resCard} /></Link> ))}
